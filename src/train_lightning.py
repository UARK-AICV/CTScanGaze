...
class CTSearcherLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {'gt_fix_vectors': gt_fix_vectors, 'predict_fix_vectors': predict_fix_vectors}
    ...
    def on_validation_epoch_end(self):
        ...
        for output in self.validation_step_outputs:
            ...
            for idx in range(num_images):
                ...
                all_predict_fix_vectors.append(output['predict_fix_vectors'][start_idx:end_idx])
        ...
        for metrics_key in cur_metrics.keys():
            for metric_name, metric_value in cur_metrics[metrics_key].items():
                self.log(
                    ...
                    metric_value,
                    on_epoch=True,
                    prog_bar=(metrics_key == 'ScanMatch' and metric_name in ['w/ duration', 'w/o duration']),
                )
...
def main():
    ...
    if num_nodes > 1:
        # Multi-node training
        strategy = 'ddp'
        print(f"🚀 Multi-node training: {num_nodes} nodes × {num_gpus} GPUs = {num_nodes * num_gpus} total GPUs")
    elif num_gpus > 1:
        ...
        # Single GPU or CPU
        strategy = 'auto'
        print(f"🚀 Single device training")
