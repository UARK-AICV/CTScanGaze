...
def main():
    # setup logger
    if os.path.exists(args.log_root) and os.path.exists(
        os.path.join(args.log_root, "checkpoints/checkpoint.pth")
    ):
    ...
    checkpoint_manager = CheckpointManager(
        model, optimizer, checkpoints_dir, mode="max", best_metric=best_metric
    )
    ...
    def validation(iteration):
        ...
        with tqdm(total=len(validation_loader) * repeat_num) as pbar_val:
            for i_batch, batch in enumerate(validation_loader):
                ...
                for trial in range(repeat_num):
                    ...
                    durations = samples["durations"]
                    sample_actions = samples["selected_actions"]
                    sampling_random_predict_fix_vectors, _, _ = (
                        sampling.generate_scanpath(
                            images, prob_sample_actions, durations, sample_actions
                        )
                    )
        ...
        logger.info(
            "Evaluation metrics after iteration {iteration}:".format(
                iteration=iteration
            )
        )
    ...
    for epoch in range(start_epoch + 1, args.epoch):
        ...
        if epoch > args.no_eval_epoch:
            ...
            tensorboard_writer.add_scalar(
                "current metric", float(cur_metric), iteration
            )
            logger.info(
                "{key:10}: {value:.4f}".format(
